    ...
    <div class="col-10">
       <div class="q-mt-lg q-ml-xs" v-for="(lockquestn, indx) in Jsoncontent.Questions" v-bind:key="indx" 
         ...
         <span class="q-mt-lg q-title">  {{indx+1}}.  {{lockquestn.description}} </span>
          <br/>                  
          <q-btn-toggle toggle-color="secondary" color="warning" class="q-mt-sm q-ml-xs" push @input="OptionAction(indx)" 
...
Vue.config.errorHandler = function (err,vm,info) {
    ...
    CommonLogging('App','error','FlatTireComponent','user1@gmail.com','115.110.71.246',err)
    Vue.$log.debug('Exception has been logged serverside.')
}

import { QBtn, QIcon, QInput, QRadio, QPopover } from 'quasar'
//import Jsoncontent from 'src/store/FlatTireQuestionaires.json'
...
export default {
  ...
  mounted()
  {
  // call API to retrieve the question details
  var vm = this   
    axios.get(process.env.API + 'questionnaires/flattire/')
    ...
    OptionAction(indx)
    {
     ...
     //First - check the popups
      for (var i in this.Jsoncontent.Popups) {               
        if (this.Jsoncontent.Popups[i].id == lnks[0]) {
            this.PopupText = this.Jsoncontent.Popups[i].description
            this.showModal =true
            ispopup = true     
        }       
      }
      //if (ispopup == true && lnks.length == 1)
      //{
